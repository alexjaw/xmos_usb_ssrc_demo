...
int g_samps_from_host[SSRC_THREADS][NUM_SSRC_SAMPS_IN][NUM_CHANNELS_PER_SSRC];     //Single buffers for input
...
static inline unsigned trigger_dsp(streaming chanend c_dsp[SSRC_THREADS],
        int samps_from_host[SSRC_THREADS][NUM_SSRC_SAMPS_IN][NUM_CHANNELS_PER_SSRC], unsigned sample_rate_code)
{
    unsigned pull_count;
#pragma loop unroll
...
#pragma loop unroll
        for (int j=0; j<NUM_SSRC_SAMPS_IN; j++){                    //Send to each engine
#pragma loop unroll
...
void src_manager_task(chanend c_host, chanend c_i2s, streaming chanend c_dsp[SSRC_THREADS], chanend c_i2s_sampfreq)
{
    ...
    unsigned samples_idx = 0;   //Index for counting NUM_SSRC_SAMPS_IN sets of samples
...
#pragma loop unroll
                for (int chan_i=0; chan_i<NUM_USB_CHAN_OUT; chan_i++){  //Get samples from host
                    ...
                    if (chan_i==0) xscope_int(FROM_DECOUPLE, samp);
            ...
            ////////////////////Call SSRC here when we have NUM_SSRC_SAMPS_IN samples////////////////
            samples_idx++;
            if (samples_idx == NUM_SSRC_SAMPS_IN){
                samples_idx = 0;
                t :> t0;
...
void dsp_task(streaming chanend c_dsp, unsigned instance_id)
{
    ...
    int        in_buff[NUM_SSRC_SAMPS_IN * NUM_CHANNELS_PER_SSRC];
    int        out_buff[NUM_SSRC_SAMPS_IN * SSRC_N_IN_OUT_RATIO_MAX * NUM_CHANNELS_PER_SSRC];
    ...
    memset(out_buff, 0, NUM_SSRC_SAMPS_IN * SSRC_N_IN_OUT_RATIO_MAX * NUM_CHANNELS_PER_SSRC * 4);
    //printstrln("DSP_task");
    ...
    while(1){
        ...
        c_dsp :> sr_in_out_new;
        n_samps_in += NUM_SSRC_SAMPS_IN;

        for(unsigned i=0; i<NUM_SSRC_SAMPS_IN * NUM_CHANNELS_PER_SSRC; i++) {
            int tmp;
            c_dsp :> tmp;
            in_buff[i] = tmp;
        }
